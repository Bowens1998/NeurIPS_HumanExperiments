...
(async function () {
  ...
  function newTrial() {
    const trialIdx = runLog ? runLog.trials.length : 0;
    const originalRandom = Math.random;
    Math.random = mulberry32(12345 + trialIdx * 999);

    // Hardcode parameters for 3 difficulty conditions
    // Condition 1 (Easy): Low prev, low noise, low vol
    // Condition 2 (Medium): High prev, low noise, low vol
    // Condition 3 (Hard): High prev, high noise, high vol
    if (trialIdx === 0) {
        controls.prevalence.value = 'low';
        controls.noise.value = 'low';
        controls.vol.value = 'low';
    } else if (trialIdx === 1) {
        controls.prevalence.value = 'high';
        controls.noise.value = 'low';
        controls.vol.value = 'low';
    } else { // 2
        controls.prevalence.value = 'high';
        controls.noise.value = 'high';
        controls.vol.value = 'high';
    }
  ...
  function stepTick(first = false) {
    ...
    const trialIdx = runLog ? runLog.trials.length : 0;
    const originalRandom = Math.random;
    Math.random = mulberry32(12345 + trialIdx * 999 + tickIdx * 777);
